...
run_plink_check_sex<-function(plink_path,bfiles_prefix,female_thresh=0.2,male_thresh=0.8,max_y_for_female=5,min_y_for_male=1){
  # first get the F stats
  check_sex_command<-glue('{plink_path} --check-sex ycount {female_thresh} {male_thresh} {max_y_for_female} {min_y_for_male} --bfile {bfiles_prefix} --out {bfiles_prefix}')
...
check_for_gender_outliers<-function(check_sex_output){
  # calculate Z score
  check_sex_output<-check_sex_output%>%group_by(SNPSEX)%>%reframe(id=IID,F=F,YCOUNT=YCOUNT,z=(F-mean(F))/sd(F))
